    ...
    <div className="flex flex-col items-center justify-center min-h-[75vh] px-4 animate-float">
      ...
      <div className="max-w-md w-full glass rounded-3xl p-8 md:p-12 border border-slate-200 dark:border-white/5 shadow-2xl relative z-10 space-y-8">
        ...
        <div className="text-center space-y-2">
          <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full border border-cyan-400/20 bg-cyan-950/20 text-cyan-500 dark:text-cyan-400 text-[10px] font-black uppercase tracking-widest">
        ...
        <form onSubmit={handleFormSubmit} className="space-y-4">
            ...
            <div className="space-y-1">
                <label className="text-[10px] uppercase tracking-widest text-slate-400 dark:text-white/40 font-bold block" htmlFor="identifier">Identifier</label>
                <input 
                  id="identifier"
                  type="text" 
            ...
            <div className="space-y-1">
                <label className="text-[10px] uppercase tracking-widest text-slate-400 dark:text-white/40 font-bold block" htmlFor="password">Credential</label>
                <input 
                  id="password"
                  type="password" 
                  placeholder="••••••••••••" 
        ...
        <div className="relative flex items-center justify-center">
            ...
            <span className="relative px-3 bg-white dark:bg-cyber-black text-[9px] font-mono text-slate-400 dark:text-white/40 uppercase tracking-widest">Federated Access</span>
        </div>

        {/* Federated Sign-In Button */}
        <button 
            onClick={handleGoogleSignIn}
            className="w-full flex items-center justify-center bg-slate-100/50 hover:bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 dark:hover:bg-white/10 hover:border-cyan-500/30 dark:hover:border-cyan-400/20 text-slate-700 dark:text-white/80 hover:text-slate-900 dark:hover:text-white font-extrabold text-xs uppercase tracking-widest py-3.5 rounded-xl transition-all duration-300 flex items-center justify-center space-x-2"
        >
            {ICONS.google}
        ...
        <p className="text-center text-xs text-slate-500 dark:text-white/50 pt-2 font-mono">
            New telemetry node?{' '}
            <button onClick={onSwitchToRegister} className="font-extrabold text-cyan-600 dark:text-cyan-400 hover:text-cyan-500 dark:hover:text-cyan-300 hover:underline uppercase text-[11px] tracking-wider transition-colors ml-1 focus:outline-none">
                Register Node &gt;
            </button>
        </p>
