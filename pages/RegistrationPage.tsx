    ...
    <div className="flex flex-col items-center justify-center min-h-[75vh] px-4 animate-float">
      ...
      <div className="max-w-md w-full glass rounded-3xl p-8 md:p-12 border border-white/5 shadow-2xl relative z-10 space-y-8">
         ...
         <div className="text-center space-y-2">
            <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full border border-purple-400/20 bg-purple-950/20 text-purple-400 text-[10px] font-black uppercase tracking-widest">
        ...
        <div className="space-y-4">
          {registrationStep === 'details' ? (
            <form onSubmit={handleDetailsSubmit} className="space-y-4">
                ...
                <div className="flex rounded-xl p-1 bg-white/5 border border-white/5">
                    <button 
                      ...
                      className={`w-1/2 py-2 rounded-lg text-xs uppercase font-extrabold tracking-wider transition ${registrationType === 'email' ? 'bg-gradient-to-r from-cyan-500 to-indigo-600 text-white shadow-lg' : 'text-white/60'}`}
                    >
                      Email Channel
                    </button>
                    <button 
                      ...
                      className={`w-1/2 py-2 rounded-lg text-xs uppercase font-extrabold tracking-wider transition ${registrationType === 'phone' ? 'bg-gradient-to-r from-cyan-500 to-indigo-600 text-white shadow-lg' : 'text-white/60'}`}
                    >
                      Phone Channel
                    </button>
                </div>
                ...
                <div className="space-y-1">
                  <label className="text-[10px] uppercase tracking-widest text-white/40 font-bold block" htmlFor="destination">
                ...
                <div className="space-y-1">
                  <label className="text-[10px] uppercase tracking-widest text-white/40 font-bold block" htmlFor="username">Node Nickname</label>
                  <input 
                    id="username"
                    type="text" 
                ...
                <div className="space-y-1">
                  <label className="text-[10px] uppercase tracking-widest text-white/40 font-bold block" htmlFor="password">Gate Password</label>
                  <input 
                    id="password"
                    type="password" 
                    placeholder="••••••••••••" 
                ...
                <div className="space-y-1">
                  <label className="text-[10px] uppercase tracking-widest text-white/40 font-bold block" htmlFor="confirmPassword">Confirm Password</label>
                  <input 
                    id="confirmPassword"
                    type="password" 
                    placeholder="••••••••••••" 
            ...
            <form onSubmit={handleOtpSubmit} className="space-y-6">
                ...
                {otpMessage && (
                  <p className="text-green-400 text-xs font-mono text-center bg-green-500/10 p-2.5 rounded-xl border border-green-500/20 animate-pulse">
                    {otpMessage}
                  </p>
                )}
                ...
                <div className="flex justify-center space-x-2">
                    {otp.map((digit, index) => (
                        <input
                            ...
                            className="w-11 h-12 text-center text-xl font-bold bg-white/5 border border-white/10 rounded-lg text-white focus:ring-1 focus:ring-cyan-400 focus:border-cyan-400 focus:outline-none transition-all font-mono"
                ...
                <p className="text-center text-xs">
                    <button 
                      ...
                      className="font-black text-cyan-400 hover:text-cyan-300 hover:underline uppercase tracking-widest text-[10px]"
                    >
          ...
          {registrationStep === 'details' && (
            <p className="text-center text-xs text-white/50 pt-2 font-mono">
                Already registered?{' '}
                <button onClick={onSwitchToLogin} className="font-extrabold text-cyan-400 hover:text-cyan-300 hover:underline uppercase text-[11px] tracking-wider transition-colors ml-1 focus:outline-none">
                    Login Node &gt;
                </button>
            </p>
          )}
